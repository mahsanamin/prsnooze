...
class Queue extends EventEmitter {
  ...
  setConcurrency(value) {
    ...
    if (!Number.isFinite(parsed) || parsed < 1) throw new Error("concurrency must be at least 1");
  ...
  async _runJob(job) {
    ...
    try {
      ...
      if (e && e.skip) {
        // A runner can bail for a reason that is not a fault: the PR merged
        // while the job sat in the queue, say. End the job the same way a
        // runner that emits "skipped" and returns does, state "done", so the
        // UI shows a skip rather than a red failure the user has to look at.
        helpers.emit({
          kind: "skipped",
          reason: e.skipReason || e.code || "skipped",
          message: e.skipMessage || e.message,
          detail: e.skipDetail || "",
        });
        this.emit("state", { jobId: job.id, state: "done" });
        helpers.emit({ kind: "done" });
