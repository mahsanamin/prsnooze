...
function findPeer(config, needle) {
  ...
  if (!key) return null;
  return (
    config.peers.find((p) => String(p.name || "").toLowerCase() === key) ||
    config.peers.find((p) => String(p.shortId || "").toLowerCase() === key) ||
    config.peers.find((p) => String(p.instanceId || "").toLowerCase() === key) ||
    config.peers.find((p) => {
      try {
