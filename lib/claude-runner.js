...
// Claude occasionally adds informational stream events which have no place in
// the live review log. Keep known silent events quiet, while preserving the
// existing `other` fallback for genuinely new schema shapes.
const KNOWN_CLAUDE_EVENT_TYPES = new Set([
  "system",
  "assistant",
  "user",
  "result",
...
function isRecognizedClaudeEvent(obj) {
  return !!obj && typeof obj === "object" && KNOWN_CLAUDE_EVENT_TYPES.has(obj.type);
}
...
function runClaude({
  ...
  function handleLine(line) {
    ...
    if (!isRecognizedClaudeEvent(obj)) ee.emit("event", { kind: "other", raw: obj });
  }
