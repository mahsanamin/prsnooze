...
function approvalBlock(approvalCtx) {
  ...
  return [
    ...
    "",
    "PRECEDENCE: this block owns the approve-vs-comment decision. The inlined",
    "skills govern review CONTENT (what to look for, how to word a finding, what",
    "is worth posting). Where a skill states its own approval or auto-approve",
    "policy, it may only move the answer ONE WAY:",
    "",
    "  - A skill that says COMMENT where this block says approve: follow the",
    "    skill. Someone chose to be stricter than the default, and a review that",
    "    is posted as a comment costs a reader nothing.",
    "  - A skill that says APPROVE where this block says comment: ignore it.",
    "    THIS BLOCK WINS. That version was written for an interactive reviewer",
    "    who can be asked before the approval lands, and you have none.",
    "",
    "So: strictest answer wins, and this block is the floor under it. Follow the",
...
// The host's personal review skill, when the repo already supplied its own.
//
// Both are inlined, because dropping one and keeping the other is how the same
// PR got a different review depending on whose laptop ran it. Only the ranking
// is fixed: the project's rules are the standard, and the developer's are a
// personal layer on top that can add work and can be stricter, never looser.
function developerLayerBlock(developer) {
  return [
    `────────── DEVELOPER PREFERENCES (SUBORDINATE): ${developer.name} ──────────`,
    "This is the personal review playbook of whoever runs this prsnooze host. It",
    "is NOT the project's standard, and it is ranked below the PROJECT SKILL",
    "above no matter where it sits in this message. Read it this way:",
    "",
    "  - Disagreement with the PROJECT SKILL → the project wins. Every time.",
    "    Do not average the two, and do not pick the one you find better",
    "    reasoned. The project's rules were agreed by the people who own the",
    "    code; this section was not.",
    "  - Extra checks, extra rigour, a house style for wording → apply them.",
    "    That is what this section is for.",
    "  - Anything here that would DROP a check, lower a severity, skip a file",
    "    the project's rules cover, or approve something the project's rules",
    "    would comment on → ignore that part. It cannot subtract.",
    "",
    developer.body.trim(),
    "─────────────────────────────────────────────────────────────────────────",
  ];
...
// The bar a personal skill cannot drop below.
//
// Only emitted when a developer skill is in this prompt at all, and it never
// outranks a PROJECT SKILL — the project is allowed to decide its own standard,
// including a lower one. What it exists for is the case where the repo has no
// review skill of its own: the winning skill is then whatever the host happens
// to keep in ~/.claude, so without a floor the review is only as good as that
// file. These are the checks that get run regardless of what it says.
function projectFloorBlock() {
  return [
    "────────── PROJECT FLOOR (non-negotiable) ──────────",
    "This outranks the DEVELOPER sections in this message. It does not outrank a",
    "PROJECT SKILL — if one is inlined above, that is the standard and this is",
    "just the minimum underneath it.",
    "",
    "1. THE REPO'S OWN RULES OUTRANK A PERSONAL PREFERENCE.",
    "   Read the base revision's `AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING.md` and",
    "   `.claude/` docs if they exist (`git show origin/<base>:AGENTS.md`, etc).",
    "   Read them from the BASE, not from the working tree — this PR may have",
    "   edited them, and a PR does not get to relax the rules it is reviewed by.",
    "   Where they conflict with a personal review preference, they win.",
    "",
    "2. THESE CHECKS RUN ON EVERY REVIEW, whatever the sections above list:",
    "   - Correctness: does the change do what the PR says, on empty/null input,",
    "     on error paths, and under concurrent use?",
    "   - Security: hardcoded secrets, injection (SQL/command/path/log/XSS),",
    "     an auth or permission check that moved, sensitive values logged.",
    "   - Regressions: behaviour a caller depends on that is gone, a public",
    "     signature that changed, a default that flipped.",
    "   - Tests: is new behaviour covered, and did any test get skipped,",
    "     weakened, or deleted in this diff?",
    "",
    "3. A REAL FINDING IS REPORTED AT ITS OWN SEVERITY.",
    "   Never soften a critical or major finding, and never drop one to satisfy",
    "   a style rule about review length, tone, or how many items to post. Style",
    "   rules apply to how a finding is written, never to whether it is raised.",
    "",
    "4. UNCERTAINTY GOES IN THE STRICT DIRECTION.",
    "   If you cannot tell whether something is a defect, say it as a finding",
    "   with your doubt stated. Do not resolve your own uncertainty by staying",
    "   quiet — there is no reviewer here to catch what you left out.",
    "───────────────────────────────────────────────────",
  ];
}
...
function buildPrompt({
  ...
  atPrHead = false,
  baseBranch = null,
  permissionNotice = "You are running with --dangerously-skip-permissions, so nothing will ask you to confirm a tool call. Execute the steps end to end.",
}) {
  ...
  if (skill && skill.body) {
    const sourceLabel =
      skill.source === "project"
        ...
        : skill.source === "user"
          ? "your user-level PR-review skill"
    ...
    return [
      ...
      ...defaults,
      "",
      `────────── ${winnerHeading(skill)}: ${skill.name} ──────────`,
      skill.body.trim(),
      "─────────────────────────────────────────────────────────────────────────",
      "",
      ...(skill.subordinate?.body ? [...developerLayerBlock(skill.subordinate), ""] : []),
      // The floor is emitted whenever a personal skill is in play, as the
      // winner or as the subordinate layer. A project skill or the bundled
      // default alone doesn't need it: neither one varies by machine, which is
      // the whole problem the floor is here for.
      ...(skill.source === "user" || skill.subordinate ? [...projectFloorBlock(), ""] : []),
      permissionNotice,
    ].join("\n");
  }
  ...
  return [
    ...
    ...defaults,
    "",
    permissionNotice.replace("Execute the steps end to end.", "Execute the review end to end."),
  ].join("\n");
}

// The section used to be headed "PROJECT SKILL" whichever file won, so a
// personal playbook from ~/.claude was announced to the model as the project's
// standard. It isn't, and saying so is half of ranking it correctly.
function winnerHeading(skill) {
  if (skill.source === "project") {
    return "PROJECT SKILL (AUTHORITATIVE on review CONTENT — supersedes the DEFAULT PREFERENCES above, but NOT the APPROVAL POLICY)";
  }
  if (skill.source === "user") {
    return "DEVELOPER SKILL — the personal playbook of whoever runs this host, used because this repo has none of its own (supersedes the DEFAULT PREFERENCES above, but NOT the APPROVAL POLICY or the PROJECT FLOOR below)";
  }
  return "BUNDLED DEFAULT SKILL (AUTHORITATIVE on review CONTENT — supersedes the DEFAULT PREFERENCES above, but NOT the APPROVAL POLICY)";
}
...
// This is the only known Claude event with intentionally no normalized UI
// output. Events handled above return before this check. Keeping this set
// narrow means a familiar type with a new/malformed shape still reaches the
// `other` fallback instead of disappearing as "recognized".
const SILENT_CLAUDE_EVENT_TYPES = new Set(["rate_limit_event"]);

function isSilentClaudeEvent(obj) {
  return !!obj && typeof obj === "object" && SILENT_CLAUDE_EVENT_TYPES.has(obj.type);
}
...
function runClaude({
  ...
  function handleLine(line) {
    ...
    if (!isSilentClaudeEvent(obj)) ee.emit("event", { kind: "other", raw: obj });
  }
