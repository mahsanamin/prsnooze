...
function buildPrompt({ prUrl, skill }) {
  if (skill && skill.body) {
    return [
  ...
  return [
    ...
    "single, well-structured review (one `gh pr review --comment` call —",
    "not multiple). Cover correctness, security, observability, and tests.",
    "Skip nits unless the change is small.",
    "",
    "You are running with --dangerously-skip-permissions. Do not ask for",
 ...
 */
function runClaude({ prUrl, skill = null, cwd, claudeBin = "claude" }) {
  const ee = new EventEmitter();
  const prompt = buildPrompt({ prUrl, skill });
