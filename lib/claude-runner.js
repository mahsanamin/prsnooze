...
function approvalBlock(approvalCtx) {
  ...
  if (!sizeOk) {
    ...
    return [
      "APPROVAL POLICY (auto-approve disabled for this PR — too large):",
      `  ${total} lines changed across ${stats?.changedFiles ?? "?"} files`,
      `  (threshold: ≤${maxLines} lines AND ≤${maxFiles} files).`,
      "Post via `gh pr review <N> --comment --body <file>`. Do NOT approve,",
      "even if you find no issues. Large PRs need human eyes regardless.",
    ];
  }
  return [
    "APPROVAL POLICY (auto-approve enabled — small PR, criticality check):",
    "",
