...
async function request(peer, config, method, routePath, { body = null, timeoutMs = DEFAULT_TIMEOUT_MS, query = null } = {}) {
  // A token is optional: most instances leave the remote API as open as their
  // page. Send one if we have it and let the instance decide, rather than
  // refusing here and making the caller guess whether it was even needed.
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 && !token) {
      message = "this instance requires a shared token; ask its host, then run `snooze token <token>`";
    }
...
function requesterLabel() {
  if (process.env.SNOOZE_REQUESTER?.trim()) return process.env.SNOOZE_REQUESTER.trim();
  let user = "unknown";
  try { user = os.userInfo().username || user; } catch {}
  return `${user}@${os.hostname()}`;
}

const postReview = (peer, config, { prUrl, provider = null, requester = requesterLabel() }) =>
  request(peer, config, "POST", "/review", { body: { prUrl, provider, requester } });
...
const postResume = (
  ...
  config,
  jobId,
  { force = false, requester = requesterLabel() } = {},
) => request(peer, config, "POST", `/jobs/${encodeURIComponent(jobId)}/resume`, {
  body: { force, requester },
});
