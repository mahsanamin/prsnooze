...
/**
 * The PR's current state, and nothing else. One cheap `gh pr view`.
 *
 * This exists apart from fetchResumeSignals because the two questions are
 * different: resuming asks "is there new work to look at", approving only asks
 * "is this PR still open and unapproved". Resume signals are also unavailable
 * for a review with no recorded Claude session (assessResumability short-circuits
 * on NO_SESSION before it reads state), and whether a PR is merged has nothing
 * to do with whether we kept a session id.
 *
 * Never throws: an unreachable GitHub is reported as an unknown state, so the
 * caller can decide what to do with "don't know" rather than handling an error.
 */
async function fetchPrState(prUrl) {
  try {
    // Inside the try on purpose. parsePrUrl throws on a URL it can't parse, and
    // a corrupt or legacy job file must not turn a read-only probe into an
    // unhandled rejection — "never throws" is the whole contract here.
    const parsed = parsePrUrl(prUrl);
    const { stdout } = await execFileP(
      "gh",
      ["pr", "view", parsed.url, "--json", "number,state,reviewDecision,isDraft"],
    ...
    return {
      ...
      approved: String(pr.reviewDecision || "").toUpperCase() === "APPROVED",
    };
  } catch (e) {
    return {
      ok: false,
...
// Everything the forced-approve gate needs, in one read-only call that never
// throws. Two things here are not available from `gh pr view --json`, which is
// why this is GraphQL and not a third variant of the REST probes above:
//
//   - reviewThreads.isResolved — whether an inline comment has been answered.
//     The REST comments endpoint has no notion of a resolved thread at all, so
//     without this every old nit would look like an open blocker forever.
//   - reviews[].commit.oid — which commit a review body was written against,
//     so findings on code the author has since replaced don't hold up an
//     approval of the code that replaced it.
//
// Shaped into plain objects here so lib/approval-gate.js stays pure and knows
// nothing about GitHub's payloads.
const APPROVAL_SIGNALS_QUERY = `
query($owner:String!,$repo:String!,$number:Int!){
  repository(owner:$owner,name:$repo){
    pullRequest(number:$number){
      number state isDraft reviewDecision headRefOid
      author{login}
      reviewThreads(first:100){
        nodes{
          isResolved isOutdated path line originalLine
          comments(first:20){nodes{body path line originalLine author{login}}}
        }
      }
...
async function fetchApprovalSignals(prUrl) {
  try {
    // Inside the try: parsePrUrl throws on a URL it can't read, and a legacy or
    // corrupt job file must not turn a read-only probe into a crash. The gate
    // treats ok:false as "hold the approval", which is the right answer for an
    // unparseable PR URL too.
    const parsed = parsePrUrl(prUrl);
    const { stdout } = await execFileP(
      "gh",
      [
        "api", "graphql",
        "-f", `query=${APPROVAL_SIGNALS_QUERY}`,
    ...
    if (!pr) {
      const why = (payload?.errors || []).map((e) => e.message).join("; ");
      return { ok: false, error: why || "GitHub returned no pull request" };
    }
    const threads = (pr.reviewThreads?.nodes || []).filter(Boolean).map((t) => ({
      isResolved: !!t.isResolved,
      isOutdated: !!t.isOutdated,
      path: t.path || null,
      line: t.line || t.originalLine || null,
      comments: (t.comments?.nodes || []).filter(Boolean).map((c) => ({
        body: c.body || "",
    ...
    return {
      ...
      latestReviews: (pr.latestOpinionatedReviews?.nodes || []).filter(Boolean).map((r) => ({
        state: String(r.state || "").toUpperCase(),
        author: r.author?.login || null,
      })),
    };
    ...
    return {
      ok: false,
      error: (e.stderr || e.message || "gh api graphql failed").toString().trim().split("\n").slice(-2).join(" "),
    };
  }
}
