...
function runProviderProcess({ provider, job, helpers, options, log }) {
  return new Promise((resolve, reject) => {
    ...
    let lastResult = null;
    // The provider's own last words. When it exits non-zero its stderr is very
    // often empty and the real reason arrived as an event instead.
    let lastText = "";
    ...
    ee.on("event", (event) => {
      ...
      if (event.kind === "assistant_text" && event.text) lastText = event.text;
    ...
    ee.on("exit", ({ code, signal, stderrTail }) => {
      ...
      // Report the cause, not just the exit code. A review that died on
      // "Failed to authenticate: OAuth session expired and could not be
      // refreshed" used to surface as "Last stderr: (empty)", which tells the
      // host nothing they can act on: the provider had said exactly what was
      // wrong, on a channel this message ignored.
      const reported =
        (typeof lastResult?.result === "string" && lastResult.result.trim()) ||
        lastText.trim() ||
        "";
...
async function runReviewJob(job, helpers, config) {
  ...
  if (skill) {
    // The subordinate layer is recorded too. Which rules a review actually ran
    // under is the first question asked about a review anyone disagrees with,
    // and "the project's skill, plus this host's own underneath it" is a
    // different answer from "the project's skill".
    const subordinate = skill.subordinate
      ? {
          name: skill.subordinate.name,
          path: skill.subordinate.path,
          pathDisplay: tildify(skill.subordinate.path),
          source: skill.subordinate.source,
        }
      : null;
    job.skill = { name: skill.name, path: skill.path, source: skill.source, subordinate };
    helpers.emit({
      kind: "skill_resolved",
      name: skill.name,
      path: skill.path,
