...
const { runClaude, approvalBlock, workingTreeBlock } = require("./claude-runner");
...
async function runReviewJob(job, helpers, config) {
  ...
  const repoPath = await withRepoLock(repoKey, async () => {
    ...
    // So claude honors this repo's own .claude/ instead of dropping it as an
    // untrusted workspace. See lib/claude-trust.js.
    await ensureWorkspaceTrusted(rp, { onLog: (m) => log(m) });
 ...
 */
function buildVerifyPrompt({ meta, approval, checkout = null }) {
  return [
 ...
 * session (stored on disk keyed by cwd), and re-checks it out at the PR's
...
async function runVerifyJob(job, helpers, config) {
  ...
  const verifyPrompt = buildVerifyPrompt({ meta, approval, checkout: { sha: checkout.sha, atPrHead } });
