...
async function runReviewJob(job, helpers, config) {
  ...
  // If we reached here without a detected post action AND claude exited
  // cleanly, the reviewer intentionally posted nothing — every concern
  // overlapped with existing reviews. Surface that as a distinct outcome so
  // the UI doesn't show "done" with no badge (which looks like a bug).
  if (!job.outcome) {
    job.outcome = "no_new_findings";
    helpers.emit({ kind: "outcome_detected", outcome: "no_new_findings" });
    log("Reviewer finished without posting — no new findings beyond existing reviews.");
  }
...
module.exports = { runReviewJob, runVerifyJob };
