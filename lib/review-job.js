...
async function runReviewJob(job, helpers, config) {
  ...
  log(`PR #${meta.number} "${meta.title}" — base=${meta.baseRefName}, head=${meta.headRefName} (+${meta.additions}/-${meta.deletions}, ${meta.changedFiles} files)`);

  // Decide auto-approval policy based on env toggle + PR size.
  const totalLines = meta.additions + meta.deletions;
  const sizeOk =
    totalLines <= config.autoApproveMaxLines &&
    meta.changedFiles <= config.autoApproveMaxFiles;
  const approval = {
  ...
  if (!approval.autoApprove) {
    ...
    approvalReason = `PR too large (${totalLines} lines / ${meta.changedFiles} files; limit ${config.autoApproveMaxLines}/${config.autoApproveMaxFiles})`;
  } else {
    approvalReason = "eligible — final call delegated to reviewer based on criticality";
  }
  helpers.emit({
  ...
  if (skill) {
    job.skill = { name: skill.name, path: skill.path };
    helpers.emit({
      kind: "skill_resolved",
      name: skill.name,
      path: skill.path,
    ...
    log(`Using project review skill: ${skill.name} (${tildify(skill.path)})`);
  } else {
    helpers.emit({
      kind: "skill_missing",
      attempted: attempted.map((p) => tildify(p)),
    });
    log(`No project review skill found — running a generic review.`);
  }
