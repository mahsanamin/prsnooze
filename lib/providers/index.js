  ...
  claude: {
    ...
    checkAuth: async ({ bin }) => {
      ...
      try {
        ...
        // Say WHY it could not be read. This fallback firing under launchd
        // while the same command works in a login shell is the tell for a
        // service that cannot reach the stored credentials, which shows up
        // later as "OAuth session expired and could not be refreshed" on every
        // review. Swallowing the reason hid the one clue that mattered.
        const why = String(e?.stderr || e?.message || e).replace(/\s+/g, " ").trim().slice(0, 160);
        return `~/.claude/ present, but \`claude auth status\` failed here: ${why || "no output"}`;
      }
    },
