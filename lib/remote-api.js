 ...
 *
 * A token is OPTIONAL here, and unset is the default. That reads as alarming
 * until you compare it with the page: `POST /api/review` accepts an
 * unauthenticated request from anyone who can reach the host, queues a review,
 * spends that host's provider plan and posts under their GitHub identity. The
 * CLI asks for exactly the same thing by exactly the same people. Gating this
 * surface while that one stayed open was a lock on the side door of an unlocked
 * house: it bought no protection and cost every colleague a "ask the host for
 * the token" round trip.
 *
 * So the default posture matches the page. Setting PRSNOOZE_REMOTE_TOKEN gates
 * this surface, which is worth doing when the instance is reachable somewhere
 * the page is not, or alongside locking the page down. Either way the two paths
 * should agree, and the page tells the host which state they are in.
 */
function createRemoteRouter({
  ...
  // A configured token gates this surface. No token means it matches the page,
  // which is already open to whoever can reach the host.
  const required = typeof token === "string" && token.trim().length > 0;
