# PRSnooze operating guide

[Back to the quick start](../README.md). This guide covers operating an existing
installation. Run the commands below from the PRSnooze repository folder.

Keep access on a trusted private network. The web page accepts review requests
without a login, and provider commands run without permission prompts or a
sandbox. Reviews use the host's GitHub identity and provider quota or API budget.
Relevant code and prompts are sent to the selected AI provider.
...
Codex has separate authentication. Check the service's Codex preflight output
as well; a working interactive login alone does not prove a service can use it.
...
The meter is visible to everyone who can reach the page. The numbers come from
the CLI's `/usage` report, not a separate AI review request.

The same panel includes a month-to-date total from review history, with an
estimate at API rates where available. That estimate is not your subscription
bill or a measurement of how much subscription quota remains.
...
Use `bin/snooze` from the repository, or run `npm link` once to enable `snooze`
from any directory. If five people on your team run prsnooze, `snooze` lets you use all five without
...
### Docker settings beyond the provider selection

Compose reads `.env` for substitution, but only variables listed under
`environment` in `docker-compose.yml` reach the container. For settings and
approval passwords, create a local `docker-compose.override.yml` beside that file:

```yaml
services:
  prsnooze:
    environment:
      PRSNOOZE_SETTINGS_PASSWORD: "${PRSNOOZE_SETTINGS_PASSWORD:-}"
      MANUAL_APPROVE_PASSWORD: "${MANUAL_APPROVE_PASSWORD:-}"
      PRSNOOZE_HOST: "${PRSNOOZE_HOST:-}"
      AUTO_APPROVE: "${AUTO_APPROVE:-true}"
      PRSNOOZE_REMOTE_TOKEN: "${PRSNOOZE_REMOTE_TOKEN:-}"
```

Keep actual secret values only in your private `.env`, not the YAML or git.
Apply changes when no reviews are running with `bin/docker-server rebuild`.
Restarting an existing container alone does not update its environment.
...
| `PRSNOOZE_HOST` | *detected* | The name the page shows, such as "by Ada" beside the logo, and the browser tab title. Falls back to `git config user.name`, then the OS username, then the hostname. |
| `HERO_IMAGE` | `/heroes/sleepy-cat.svg` | Background image URL or bundled asset path. |
...
Risky or large PRs come back as *commented* on purpose. The merge decision stays
with a human. Eligible finished reviews show an **Approve PR** button, gated by
a shared password. It is not offered for known merged, closed, or already
approved PRs, or for your own PR.

Clicking the button confirms what's about to happen, then asks for the password
**every time**. An unlocked settings session does not authorize approval.
